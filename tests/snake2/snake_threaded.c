...
#define MAXPUNTOS			7
#define TICK_INTERVAL		20		// Tiempo de muestreo de la tableta en milisegundos
...
void WacomTimer(int value) {
	...
	glutTimerFunc(TICK_INTERVAL, WacomTimer, 0);
	...
	fprintf(archivo,"%d \t %d  \t %d \t %d \t %d \t %ld \%ld \n", 
					g_bufferx[contador], g_buffery[contador],
...
void OnInit() {
	...
	fprintf(archivo,"X \t Y \t WacX \t WacY \t Press \t Seconds \t MicroSec \n");
	WacomInit(NULL);
