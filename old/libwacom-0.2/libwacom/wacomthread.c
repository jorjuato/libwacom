...
//------------------------------------------------------------  WacomInit()
//
void WacomInit(int mode, void (*WacomCallback)(int)) {
	if ( !InitAll() ) {
	...
	if (mode == WACOM_SCANMODE_POLL) {
		if ( pthread_create(&g_thread_id,NULL,WS_Thread,NULL) )	{
	...
	else if (mode == WACOM_SCANMODE_UNBLOCK) {
	...
	if (WacomCallback)	
		SetWacomCallback(WacomCallback);
}
...
static void SetWacomCallback(void (*WacomCallback)(int)) {
	...
	memset(&sighandler, 0, sizeof(sighandler));
	sighandler.sa_handler = WacomCallback;
...
static int WS_SetTimer()
{
	...
	signal(SIGALRM, WS_Timer);			 //Sets the alarm handler
